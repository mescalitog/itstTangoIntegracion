...
/*
 * Este modulo se puede llamar mediante un cron job para sincronizar productos
 */
...
require_once dirname(__FILE__) . '/classes/stock/productos.php';

use ItSt\PrestaShop\Tango\Constantes as Consts;
use ItSt\PrestaShop\Tango\Helpers as Helpers;
use ItSt\PrestaShop\Tango\Stock as Stock;
...
$logger->addLog(
    'products-cron;started;token=' . $token
        . ', secureKey=' . Helpers\ItStTools::getSecureKey()
        . ', id_shop=' . $id_shop,
...
if (Helpers\ItStTools::validateSecureKey($token)) {
    return Stock\Productos::instance()->syncProducts($id_shop);
} else {
    $logger->addLog(
        'products-cron;token and secureKey don\'t match',
